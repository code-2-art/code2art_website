...
> 本报告为 `project_readme_claude.md`（2026-02-22）的更新版，反映了 Claude 辅助重构后的最新项目状态。
...
项目以**飞书 (Feishu/Lark) 多维表格 (Bitable)** 作为内容管理后台，数据通过两条路径进入前端：
...
| `scripts/feishu-common.js` | **共享工具模块**：Bitable API 调用、字段映射、文件下载、记录解析 | — | — |
| `scripts/feishu-doc-util.js` | 底层工具：认证、Doc 内容获取、链接解析 | — | — |
...
├── nginx.sample.conf       # Nginx 部署参考配置（已修复为 MPA fallback）
├── .env.example            # 环境变量模板（含活动表配置与功能开关）
│
├── scripts/                # 预构建 ETL 脚本
│   ├── feishu-doc-util.js  # 底层工具：飞书认证、Doc HTML 获取
│   ├── feishu-common.js    # ★ 共享模块：API 调用、字段解析、文件下载、记录映射
...
│   ├── lib/
│   │   ├── feishuBase.ts   # ★ 共享数据层：认证、API 调用、字段解析、JSON 读取
│   │   ├── feishu.ts       # 博客数据层（Post 类型、getPosts、getPostBySlug）
│   │   └── feishuActivities.ts # 活动数据层（Activity 类型、getActivities）
│   └── pages/
│       ├── index.astro     # 首页（单页长滚动，含所有 section）
│       ├── blog/
│       │   ├── index.astro # 博客列表页
...
## 4. 重构改进记录（相比 2026-02-22 版本）

本次重构由 Claude 辅助完成（commit `e6a90fb`），主要解决了前版分析中提出的多项问题：

### 4.1 已修复的问题

| # | 问题 | 修复方式 |
|---|------|----------|
| 1 | `.gitignore` 合并冲突残留 | ✅ 已清理冲突标记 |
| 2 | `package.json` 冗余依赖 `"dotevn"` | ✅ 已移除，仅保留正确的 `"dotenv": "^17.2.2"` |
| 3 | `.env.example` 不完整 | ✅ 已补充活动表配置 (`FEISHU_ACTIVITIES_*`) 和功能开关 (`USE_FEEDS`, `DOWNLOAD_ASSETS`) |
| 4 | Nginx 配置 SPA fallback 不合理 | ✅ 已修改为 `try_files $uri $uri/ =404`（MPA 风格） |
| 5 | 前端数据层代码重复 | ✅ 新增 `src/lib/feishuBase.ts`（119 行），抽取共享逻辑：认证、API 调用、字段解析、JSON 读取、排序工具函数 |
| 6 | ETL 脚本代码重复 | ✅ 新增 `scripts/feishu-common.js`（131 行），抽取共享逻辑：API 调用、字段映射、文件下载、记录映射 |

### 4.2 重构后的架构对比

**重构前：**
```
feishu.ts (≈150 行)          ←  70% 重复  →  feishuActivities.ts (≈140 行)
feishu-fetch.js (≈120 行)    ←  大量重复  →  fetch-activities.js (≈100 行)
```

**重构后：**
```
feishuBase.ts (119 行，共享)
  ├── feishu.ts (60 行，仅含 Post 类型 + getPosts/getPostBySlug)
  └── feishuActivities.ts (59 行，仅含 Activity 类型 + getActivities)

feishu-common.js (131 行，共享) + feishu-doc-util.js (45 行，底层)
  ├── feishu-fetch.js (86 行，博客数据抓取)
  └── fetch-activities.js (59 行，活动数据抓取)
```

代码总行数显著减少，消除了大量重复逻辑。

## 5. 内容结构 (Content Structure)

### 5.1 首页 (`src/pages/index.astro`，432 行)
...
| **Hero** | 品牌宣言「艺术是算法的叛徒」（渐变文字）；中英文社群介绍；双 CTA 按钮 |
| **Activities** | 展示最新 3 条活动卡片，链接跳转至飞书/微信外链 (`docUrl`) |
| **Blog** | 展示最新 3 篇博客卡片，链接跳转至外链；底部"查看更多"链接到 `/blog/` |
| **Contact** | 企业微信加群入口 + 联系方式 |
| **Footer** | 社交平台链接矩阵（公众号、小红书、B站、抖音）+ 资源链接（小鹅通、GitHub、Gumroad、YouTube） |
| **Back-to-top** | 固定右下角返回顶部按钮（滚动 > 300px 时显示） |
...
- 链接优先使用 `docUrl`（外链），保留 `/blog/[slug]` 作为备用

### 5.3 活动板块

- **列表页** (`/activities/`)：带分页（每页 9 条），三栏网格布局，URL 参数分页 (`?page=2`)
- **详情页** (`/activities/[slug]`)：SSG 动态路由，展示活动封面、日期、作者、正文

### 5.4 链接策略

首页和列表页的卡片**优先链接到飞书/微信外链** (`docUrl`)，当 `docUrl` 为空时回退到站内路由 (`/blog/[slug]` 或 `/activities/[slug]`)。代码中保留了纯站内链接的注释代码，便于未来切换。
...
两者字段高度相似，主要区别是 Activity 使用 `date` 而非 `publishedAt`。共享字段的解析逻辑已统一抽取到 `feishuBase.ts` 和 `feishu-common.js`。
...
- 监听 80 端口
- 静态资源 30 天缓存
- MPA 风格 fallback (`try_files $uri $uri/ =404`)
...
- 响应式设计（移动端汉堡菜单、网格自适应列数：1→2→3 列）

## 9. 源文件行数统计

| 路径 | 行数 | 类型 |
|------|------|------|
| `src/pages/index.astro` | 432 | 首页模板 |
| `src/pages/activities/index.astro` | 102 | 活动列表页 |
| `src/pages/activities/[slug].astro` | 54 | 活动详情页 |
| `src/pages/blog/index.astro` | 36 | 博客列表页 |
| `src/pages/blog/[slug].astro` | 35 | 博客详情页 |
| `src/pages/api/health.json.ts` | 3 | 健康检查 |
| `src/layouts/BaseLayout.astro` | 22 | 全局布局 |
| `src/styles/tailwind.css` | 21 | 自定义样式 |
| `src/lib/feishuBase.ts` | 119 | 共享数据层 |
| `src/lib/feishu.ts` | 60 | 博客数据层 |
| `src/lib/feishuActivities.ts` | 59 | 活动数据层 |
| `src/env.d.ts` | 12 | 类型声明 |
| `scripts/feishu-common.js` | 131 | ETL 共享模块 |
| `scripts/feishu-fetch.js` | 86 | 博客数据抓取 |
| `scripts/fetch-activities.js` | 59 | 活动数据抓取 |
| `scripts/feishu-assets.js` | 47 | 图片本地化 |
| `scripts/feishu-doc-util.js` | 45 | 底层工具 |

## 10. 当前仍存在的待改进点

1. **详情页链接策略过渡态**：首页卡片和列表页优先链接到外链 (`docUrl`)，但站内详情页 (`/blog/[slug]`, `/activities/[slug]`) 仍会被 SSG 生成。两套逻辑并存，建议明确最终策略。

2. **Health 端点在纯静态模式下的意义**：`api/health.json.ts` 在 `output: 'static'` 模式下被预渲染为静态 JSON 文件，仅能验证构建是否成功，无法反映运行时状态。

3. **飞书 Docx 不支持自动转 HTML**：当前仅支持旧版飞书 Doc 的 HTML 自动获取，新版 Docx 需要手动粘贴 HTML 到 Bitable 字段。

4. **`feishu-assets.js` 中 `rewriteHtmlImages()` 未实现**：正文 HTML 中的远程图片尚未自动本地化。

5. **双包管理器锁文件并存**：`package-lock.json` 和 `pnpm-lock.yaml` 同时存在，建议统一使用一种包管理器。
