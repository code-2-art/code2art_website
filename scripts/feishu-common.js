// scripts/feishu-common.js — 脚本层共享工具函数
import fs from 'node:fs';
import path from 'node:path';
import { API_BASE, parseDocLink, getTenantAccessToken, fetchDocHtml } from './feishu-doc-util.js';

export { API_BASE, parseDocLink, getTenantAccessToken, fetchDocHtml };
...
export async function listAllRecords({ appToken, tableId, viewId }) {
...
export async function download(url, destDir) {
  const u = new URL(url);
...
export async function enrichContentHtmlFromDocIfNeeded(item, token) {
...
export function mapRecordBase(rec, contentHtmlResolved) {
